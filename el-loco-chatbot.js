...
// Version 3.3 - Corrected Price Logic & Parts List Link
...
(function() {
    ...
    function searchVehicles(searchQuery) {
        ...
        const makeModelPatterns = {
            'accord': ['honda', 'accord'], 'accords': ['honda', 'accord'], 'civic': ['honda', 'civic'], 'civics': ['honda', 'civic'], 'camry': ['toyota', 'camry'], 'camrys': ['toyota', 'camry'], 'corolla': ['toyota', 'corolla'], 'corollas': ['toyota', 'corolla'], 'f150': ['ford', 'f150'], 'f-150': ['ford', 'f150'], 'silverado': ['chevrolet', 'silverado'], 'silverados': ['chevrolet', 'silverado'], 'impala': ['chevrolet', 'impala'], 'impalas': ['chevrolet', 'impala'], 'tahoe': ['chevrolet', 'tahoe'], 'tahoes': ['chevrolet', 'tahoe'], 'suburban': ['chevrolet', 'suburban'], 'suburbans': ['chevrolet', 'suburban'], 'explorer': ['ford', 'explorer'], 'explorers': ['ford', 'explorer'], 'expedition': ['ford', 'expedition'], 'expeditions': ['ford', 'expedition'], 'ranger': ['ford', 'ranger'], 'rangers': ['ford', 'ranger'], 'altima': ['nissan', 'altima'], 'altimas': ['nissan', 'altima'], 'sentra': ['nissan', 'sentra'], 'sentras': ['nissan', 'sentra'], 'maxima': ['nissan', 'maxima'], 'maximas': ['nissan', 'maxima']
        };
    ...
    function processMessage(msg) {
        ...
        if (lower.includes('direction') || lower.includes('dirección') || lower.includes('location') || lower.includes('ubicación') || lower.includes('address')) {
            return `📍 <strong>${CONFIG.ADDRESS}</strong><br><br>` +
                   `<a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(CONFIG.ADDRESS)}" target="_blank" style="color:#4a8b6b;">` +
                   `🗺️ ${t.getDirectionsLink}</a><br><br>` +
                   `📞 ${t.callUs}: <a href="tel:${CONFIG.PHONE}" style="color:#4a8b6b;">${CONFIG.PHONE}</a>`;
        }

        // FIX #1 & #2: Handle parts prices first and provide the correct link.
        // This block is moved before the vehicle search and now includes the link you wanted.
        if (lower.includes('price') || lower.includes('precio') || lower.includes('part') || lower.includes('parte') || lower.includes('cost') || lower.includes('how much')) {
            return `💰 You can find our complete, up-to-date parts list and prices right here:<br><br>` +
                   `<a href="https://www.locosgringospicknpull.com/parts-list" target="_blank" style="color:#4a8b6b; font-weight:bold; text-decoration:underline;">View Full Parts & Price List</a><br><br>` +
                   `Here are a few popular examples:<br>` +
        ...
        // FIX #1: Vehicle search now runs AFTER the price check has been attempted.
        if (inventoryData.length > 0) {
